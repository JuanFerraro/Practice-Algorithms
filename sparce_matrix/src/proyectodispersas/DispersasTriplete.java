...
public class DispersasTriplete {
	...
	public DispersasTriplete AumentarTamaño(int aumento,int mat[][],int datos,DispersasTriplete M1) {
	...
	public void SumaDosMatrices(DispersasTriplete M1,DispersasTriplete M2,DispersasTriplete M3) {
		int aumento=1,k=1,h=1,s=1,fila=0,colum=0,dato=0,datos=0,bandera=0,j=1;
		...
		while(h<=M2.getDatos()) {
			...
			if(bandera==0) {
				M3=M3.AumentarTamaño(aumento, M3.getMat(), M3.getDatos(), M3);
	...
	//------------------------------------------------------------------------------------------------------------------------------------------------------
      //MULTIPLY TWO MATZ
        public void MultiplicarDosMatrices(DispersasTriplete M1,DispersasTriplete M2,DispersasTriplete M4){ 
          int aumento=1,k1=1,FA=0,k2=1,Aux1=0,Aux2=0,L=0,band=0,Acu=0,RM=0,w=1,Fila=0,Colum=0,Datos=0,P=0;
		M4.DispersasTriplete(M1.getFilas(), M2.getColumnas(), 0);
		int mat1[][]=M1.getMat(),mat2[][]=M2.getMat(),mat4[][],j=0;
		mat4=new int[20][3];
		mat4[0][0]=mat1[0][0];
		mat4[0][1]=mat2[0][1];

                while(k1 <= M1.getDatos()){
                      FA= mat1[k1][0];
                      Fila=mat1[k1][0];
                      Colum=mat1[k1][1];
                      ...
                      L=0;
                       if(mat1[k1][0] == FA){
                           k2=1;
                           Acu=0;
                          while(k2 <= M2.getDatos()){ 
                              if(mat2[k2][1] == L){
                                   RM=mat1[k1][2] *  mat2[k2][2];
                                  Acu=Acu+RM; 
                                  mat4[w][0]=Fila;
                                  mat4[w][1]=Colum;
                                  mat4[w][2]=Acu;
		...
		M4.setMat(mat4);
               ...
               M4.MostrarMatriz(M4.getMat(),M4.getDatos()); 
      }
}